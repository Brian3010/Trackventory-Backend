...
namespace trackventory_backend.Services
{
  public class ExcelConverter : IExcelConverter
  {
    ...
    public async Task<byte[]> GenerateExcelFileAsync(List<InventoryCount> inventoryCounts) {
      ...
      worksheet.Cell(1, 5).Value = "OnHand";
      worksheet.Cell(1, 6).Value = "Counted";
      worksheet.Cell(1, 7).Value = "Quantity";
      worksheet.Cell(1, 8).Value = "Counting Reason Code";

      // Add Data
      for (int i = 0; i < inventoryCounts.Count; i++) {
        worksheet.Cell(i + 2, 1).Value = inventoryCounts[i].UpdatedDate.Date;
        worksheet.Cell(i + 2, 2).Value = inventoryCounts[i].Product.ProductName;
        worksheet.Cell(i + 2, 3).Value = inventoryCounts[i].Product.Site;
        worksheet.Cell(i + 2, 4).Value = inventoryCounts[i].Product.Warehouse;
        worksheet.Cell(i + 2, 5).Value = inventoryCounts[i].OnHand;
        worksheet.Cell(i + 2, 6).Value = inventoryCounts[i].Counted;
        worksheet.Cell(i + 2, 7).Value = inventoryCounts[i].Quantity;
        worksheet.Cell(i + 2, 8).Value = inventoryCounts[i].CountingReasonCode;
      }
